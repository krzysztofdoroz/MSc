...
public class GATest3Stocks {
	...
	public void init() {
		...
		filenames.add("/home/krzysztof/MSc/data-source/pko.data");
	...
	public void testGA() {
		...
		GeneticAlgorithmImpl gen = new GeneticAlgorithmImpl(3, 16, 0.2, 0.1,
				0.3, dataSource);

		for (int i = 0; i < 253; i++) {
			Portfolio bestPortfolio = gen.calculateCurrentPortfolio();

			if (stocskBought[0] + stocskBought[1] + stocskBought[2] > 0) {
				money = 0;
				int index = 0;
